function InteractingNonStationaryGrowth_ParticlePlot(vg,ps,slope,tt,xlab)
...
figure(4);imagesc(reshape(x,sqrt(ps.setup.dimState),sqrt(ps.setup.dimState)));
title(['Time step: ', num2str(tt), ': true state']);colorbar;
xlabel('x');
ylabel('y');
...
ylabel('State dimension');
movegui('north');
figure(5);imagesc(reshape(vg.xp_m,sqrt(ps.setup.dimState),sqrt(ps.setup.dimState)));
