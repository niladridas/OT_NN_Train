function ps = initializePS(algs_executed,example_name)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ps.setup = struct(...
    'example_name',example_name,...%'Acoustic','Septier16','NonStationaryGrowth','InteractingNonStationaryGrowth'
    'doplot',false,...% plot the tracking trajectories
    'parallel_run',true,... % using parallel processing for different simulation trials
    'EKF',true,... % EKF/UKF for true/false
    ...
    'regularize_resample',true,...% whether to add regularization noise
    'fontSize',20,...
    ...
    'LEDH',true); % set false for EDH

%'PFPF',true,...'kflag','EKF1',...%'EKF1','regularized_identity',...'none'; % the method used to estimate the prior covariance.
...
switch ps.setup.example_name
    ...
    case 'NonStationaryGrowth'
        addpath('NonStationaryGrowth');
        ps = NonStationaryGrowth_initialization(ps);
    case 'InteractingNonStationaryGrowth'
        addpath('InteractingNonStationaryGrowth');
        ps = InteractingNonStationaryGrowth_initialization(ps);
end
