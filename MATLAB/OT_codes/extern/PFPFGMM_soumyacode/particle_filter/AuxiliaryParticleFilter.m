function [output] = AuxiliaryParticleFilter(ps,z)
% The implementation of Auxiliary PF filters.
%
% Input:
...
for tt = 1:T
    ...
    xp_prop_deterministic = ps.propparams.propagatefcn(xp,propparams_no_noise); 
    
    llh_deterministic = ps.likeparams.llh(xp_prop_deterministic,z(:,tt),ps.likeparams);
   
    logW = logW + llh_deterministic;
    logW = logW - max(logW);
    ...
    indices = resample(nParticle,wt,'stratified');
    
    xp = xp(:,indices);
    
    xp = ps.propparams.propagatefcn(xp,ps.propparams); 
    
    llh_deterministic = llh_deterministic(indices);
    llh = ps.likeparams.llh(xp,z(:,tt),ps.likeparams);
    
    logW = llh - llh_deterministic;
    logW = logW - max(logW);
    ...
    xp_m = particle_estimate(logW,xp);
...
alg_name = ['APF (', num2str(nParticle),'Particles)'];
