function [error_per_trial,error_percentile_5,error_percentile_25,error_percentile_50,...
    error_percentile_75,error_percentile_95,ESS_per_trial,avg_execution_time] = ...
...
[alg_name, ' Average error: ',num2str(mean(error_per_step_per_trial(:)))]

error_per_step = mean(error_per_step_per_trial,2);
%save([alg_name,'.mat'],'error_per_step');
error_per_trial = mean(error_per_step_per_trial,1)';
error_percentile_5 = prctile(error_per_trial,5)
error_percentile_25  =prctile(error_per_trial,25);
error_percentile_50  = median(error_per_trial);
error_percentile_75  =prctile(error_per_trial,75);
error_percentile_95  = prctile(error_per_trial,95)
error_std_dev = std(error_per_trial);

[alg_name, ' error standard deviation: ',num2str(error_std_dev)];
[alg_name, ' error 5 percentile: ',num2str(error_percentile_5)];
[alg_name, ' error 25 percentile: ',num2str(error_percentile_25)];
[alg_name, ' error 50 percentile: ',num2str(error_percentile_50)];
[alg_name, ' error 75 percentile: ',num2str(error_percentile_75)];
[alg_name, ' error 95 percentile: ',num2str(error_percentile_95)];
...
avg_execution_time = mean(execution_time_per_trial/ps.setup.T);
