...
error_struct.error_metric_per_step = zeros(1,size(output.x_est,2));
...
for tt = 1:size(output.x_est,2)
    switch ps.setup.example_name
        case {'Acoustic'}
            ...
            %error_struct.error_metric_per_step(tt) = omat(estimated_tracks,true_tracks); 
        case {'Septier16'}
...
if display_error&&isfield(ps,'trial_ix')
    disp(['Trial ', num2str(ps.trial_ix),...
        ...
        ', Computation Time: ',num2str(mean(error_struct.execution_time)/ps.setup.T),...
        ', ', alg_name])
end
