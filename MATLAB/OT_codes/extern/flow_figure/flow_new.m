...
%% particle flow
slope_LEDH  = @(lambda,x) calculateSlope_LEDH(obs,x,mu,P,R,lambda);
[xdim,nSamp] = size(x);
xpost = zeros(xdim,nSamp);
parfor i = 1:nSamp
    [ttmp,xtmp] = ode45(slope_LEDH,[0 1],x(:,i));
    xpost(:,i) = xtmp(end,:)';
end
x = xpost;
...
[slope]  = calculateSlope_LEDH(obs,x,mu,P,R,1);
scatter(x(1,:),x(2,:),75,'*');
set(gcf, 'Color', 'w');
hold on;
