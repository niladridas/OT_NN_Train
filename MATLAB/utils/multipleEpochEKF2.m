function [yEKF, NN_EKF] = multipleEpochEKF2(maxEpoch,kEnd,P0,Q,R,yMeas,iP,NN_EKF)
x0 = getParams(NN_EKF);
...
for iEp = 1:maxEpoch
    for k = 1:kEnd
        ...
        % EKF Update
        NN_EKF =  updateParams(NN_EKF,x_pr); % Update parameters of the NN
        res = yMeas(k,1) - NN_EKF(iP(k,:)'); % Innovation/ Measurement residual
        H = nntool_Jacobian(NN_EKF,iP(k,:)'); % Jacobian of measurement model w.r.t states (NN Params in this case)
    ...
    NN_EKF = updateParams(NN_EKF,x_pst); % Update parameters of the NN
    for k = 1:kEnd
        yEKF(k,iEp) =  NN_EKF(iP(k,:)');
    end
end % epoch
end % function