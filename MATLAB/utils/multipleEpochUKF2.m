function [yUKF,NN_UKF] = multipleEpochUKF2(maxEpoch,kEnd,P0,Q,R,yMeas,iP,NN_UKF)
% Algorithm 3.1 from the Ref. paper
% Augmented state = [state; process_noise; measurement_noise]; 
x0 = getParams(NN_UKF); nx = length(x0); ny = 1;
...
%         xSP_pr = xSP;
        ...
        for isp = 1:nPt
            NN_UKF = updateParams(NN_UKF,xSP_pr(:,isp)); % Update parameters of the NN
            ySP_pr(:,isp) = NN_UKF(iP(k,:)') + measSP(:,isp);
...
%         ySP_pr(:,isp) = measModel(NN_UKF,iP(k,:)');
    ...
    NN_UKF = updateParams(NN_UKF,x_pst); % Update parameters of the NN
    for k = 1:kEnd
        yUKF(k,iEp) = NN_UKF(iP(k,:)');
    end
end % epoch
